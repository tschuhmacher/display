...
GLTFLoader.prototype = {
  ...
  load: function(url, onLoad, onProgress, onError) {
    ...
    var path =
      this.path !== undefined ? this.path : LoaderUtils.extractUrlBase(url);
  ...
  return Promise.all(pendingBufferViews).then(function(bufferViews) {
    ...
    if (byteStride && byteStride !== itemBytes) {
      ...
      bufferAttribute = new THREE.InterleavedBufferAttribute(
        ib,
        itemSize,
        accessorDef.byteOffset / elementBytes,
        normalized
      );
    } else {
      if (bufferView === null) {
  ...
  return Promise.resolve(sourceURI)
    ...
    .then(function(texture) {
      ...
      if (
        ...
        console.warn(
          "GLTFLoader: Three.js does not support texture internalFormat which is different from texture format. " +
...
GLTFParser.prototype.loadMaterial = function(materialIndex) {
  ...
  if (materialExtensions[EXTENSIONS.KHR_MATERIALS_COMMON]) {
    ...
    pending.push(
      khcExtension.extendParams(materialParams, materialDef, parser)
    );
  } else if (
      ...
      if (sampler) {
        ...
        if (node) {
          ...
          if (sampler.interpolation === "CUBICSPLINE") {
            ...
            var outputAccessorValues = new TypedArray(
              (outputAccessor.count * itemSize) / 3
            );
            ...
            for (var j = 0, jl = outputAccessor.count; j < jl; j += 3) {
              outputAccessorValues[(j * itemSize) / 3] = outputAccessor.getX(
                j + 1
              );
              if (itemSize > 1)
            ...
            outputAccessor = new THREE.BufferAttribute(
              outputAccessorValues,
              itemSize / 3,
